...
def read_config(filename):
    ...
    config = {'data_dir':'./', 'mask_dir': None, 'output_dir':'./', 'sps_model_dir': None, 'filters': None, 'main_band': None, \
              ...
              'rgbcuts': None, 'outname': None, 'ebmv': None}
    ...
    if config['ebmv'] is not None:
        config['ebmv'] = float(config['ebmv'])
    else:
        config['ebmv'] = 0.
        ...
        if len(line) > 0:
            if line[0] == 'light_model':
                ...
                if sed_class == 'freecolors':
                    ...
                    sps_model = line[3]
                    parnames += ['logmass', 'redshift', 'age', 'tau', 'logZ', 'logtau_V']
                else:
                    df
                ...
                if sed_class == 'freecolors':
                    ...
                    parnames += ['logmass', 'redshift', 'age', 'logZ', 'tau', 'logtau_V']
                else:
                    df
...
class PyPLZModel:
    ...
    def __init__(self, config):
        ...
        for comp in config['light_components']:
            ...
            if comp['sed'] == 'freecolors':
                ...
                sed = SEDModels.SPS('light_sed%d'%ncomp, sed_here, self.bands, self.zp, modelname, filtnames, config['ebmv'])
            else:
                df
    ...
    def get_chi2(self):
        ...
        for light, sed in zip(self.light_sb_models, self.light_sed_models):
            ...
            light.setAmpFromMag(sed.mags[self.main_band], self.zp[self.main_band])
        ...
        for source, sed in zip(self.source_sb_models, self.source_sed_models):
            ...
            source.setAmpFromMag(sed.mags[self.main_band], self.zp[self.main_band])
    ...
    def get_restUV_mags(self):
        ...
        for light, sed in zip(self.light_sb_models, self.light_sed_models):
            if sed.__class__.__name__ == 'Template':
                main_mag = light.Mag(self.zp[self.main_band])
                scale = sed.restUV_scale(self.main_band)
                light_mags.append(main_mag - 2.5*np.log10(scale))
            else:
        ...
        for source, sed in zip(self.source_sb_models, self.source_sed_models):
            if sed.__class__.__name__ == 'Template':
                main_mag = source.Mag(self.zp[self.main_band])
                scale = sed.restUV_scale(self.main_band)
                source_mags.append(main_mag - 2.5*np.log10(scale))
            else:
    ...
    def write_config_file(self, config, outname):
        ...
        conflines = []
        confpars = ['data_dir', 'mask_dir', 'maskname', 'output_dir', 'sps_model_dir', 'filename', 'science_tag', 'err_tag', 'err_type', 'psf_tag', 'rmax', 'Nwalkers', 'Nsteps', 'burnin', 'main_band', 'filter_prefix', 'filter_suffix', 'ebmv']
        ...
        ncomp = 0
        for comp, sed, uvmag in zip(config['light_components'], self.light_sed_models, light_uvmags):
    
            if comp['sed'] == 'freecolors':
                lightpars = SBModels.parlists[comp['class']] + config['colors'] + ['main_mag']
            elif comp['sed'] == 'template':
                lightpars = SBModels.parlists[comp['class']] + ['zs', 'tn', 'main_mag']
            elif comp['sed'] == 'sps':
                lightpars = SBModels.parlists[comp['class']] + ['logmass', 'redshift', 'age', 'logZ', 'tau', 'logtau_V']
        ...
        for comp, sed, uvmag in zip(config['source_components'], self.source_sed_models, source_uvmags):
    
            if comp['sed'] == 'freecolors':
                sourcepars = SBModels.parlists['Sersic'] + config['colors'] + ['main_mag']
            elif comp['sed'] == 'template':
                sourcepars = SBModels.parlists['Sersic'] + ['zs', 'tn', 'main_mag']
